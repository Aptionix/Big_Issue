...
(ggplot(All_Data_Bang, aes(x = Bang_Trade, y = Gini_Coeff, colour = factor(flag_gi))) +
  geom_point(size = 0.4, alpha = 0.7) +      
  geom_smooth(method = "lm", size = 0.2, aes(colour = "0", fill = "0")) + 
  geom_smooth(
    method = "lm",
    size = 0.2,
    aes(x = Trade_clean, y = Gini_Coeff_clean, colour = "1", fill = "1")
  ) +
  theme_minimal(base_size = 6) +                             
  theme(
    panel.background = element_rect(fill = "#f7f7f7", color = NA), 
...
ggplot(All_Data_Bang, aes(x = Bang_Trade, y = Growth, colour = factor(flag_gr))) +
  ...
  geom_smooth(
    method = "lm",
    size = 0.2,
    aes(x = Trade_clean, y = Growth_clean, colour = "1", fill = "1")
  ) +
  theme_minimal(base_size = 6) +                             
  theme(
    panel.background = element_rect(fill = "#f7f7f7", color = NA), 
