...
(ggplot(All_Data, aes(x = Trade_clean, y = Gini_Coeff_clean, colour = Country)) +
  geom_point(size = 0.4, alpha = 0.7) +      
  geom_smooth(method = "lm", size = 0.2) + 
  theme_minimal(base_size = 6) +                             
  theme(
...
ggplot(All_Data, aes(x = Trade_clean, y = Growth_clean, colour = Country)) +
  geom_point(size = 0.4, alpha = 0.7) +     
  geom_smooth(method = "lm", size = 0.2) + 
  theme_minimal(base_size = 6) +                           
  theme(
    ...
    hjust = 0.5 )) +
  labs(
    title = "Trade/Growth (no outliers)",
    x = "Current Account ($bn)",
    y = "Growth (%)",
    colour = "Country"
  ))
