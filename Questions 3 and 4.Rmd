...
graph <- function(x, y) {
 return(ggplot(x, aes(x = Growth, y = Gini_Coeff)) +
  geom_point(color = "#1f77b4", size = 1, alpha = 0.7) +      # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 0.5) + # line: dark gray
  theme_minimal(base_size = 6) +                             # clean minimal theme
  theme(
    ...
    lineheight = 1.1,
    margin = margin(b = 10, t = 5),    # spacing above/below
    hjust = 0.5 )) +
  labs(
    title = y,
    x = "Growth (%)",
...
# Plotting the graphs of Bangladesh
...
```{r plotting1, fig.width=5, fig.height=3, dpi=200}

# Graphing Trade vs Gini for Bangladesh
(ggplot(All_Data_Bang, aes(x = Bang_Trade, y = Gini_Coeff)) +
  geom_point(color = "#1f77b4", size = 1, alpha = 0.7) +      # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1) + # line: dark gray
  theme_minimal(base_size = 6) +                             # clean minimal theme
  theme(
    panel.background = element_rect(fill = "#f7f7f7", color = NA), # light gray background
    panel.grid.major = element_line(color = "#dcdcdc"),            # soft grid lines
