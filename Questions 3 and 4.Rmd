...
Ken_W_Ineq <- read_delim("Data/Kenya_Wealth_Inequality.csv",
  delim = ";", escape_double = FALSE, col_names = FALSE,
  trim_ws = TRUE, skip = 1
) # This is why I would rather use xls file

Ken_I_Ineq <- read_delim("Data/Kenya_Income_Inequality.csv",
  delim = ";", escape_double = FALSE, col_names = FALSE,
  trim_ws = TRUE, skip = 1
)

# Importing Kenya Income Growth
Ken_I_Growth <- read_excel("Data/Bang_Income.xlsx")

# Importing Bangladesh Inequality
Bang_W_Ineq <- read_excel("Data/Bang_Wealth_Inequality.xlsx", col_names = FALSE)
Bang_I_Ineq <- read_excel("Data/Bang_Income_Inequality.xlsx", col_names = FALSE)

# Importing Bangladesh Income Growth
Bang_I_Growth <- read_excel("Data/Kenya_Income.xlsx")

# Importing Kenya and Bangladesh Trade
Trade_df <- read_excel("Data/Bang_and_Ken_Trade.xlsx", col_names = TRUE)
```
...
clean_data_growth <- function(x) {
  ...
  x <- x %>%
    mutate("Growth" = ((`National Income` - lag(`National Income`)) / lag(`National Income`) * 100)) %>%
    filter(!is.na(Growth))
...
graph <- function(x, y) {
  return(ggplot(x, aes(x = Growth, y = Gini_Coeff)) +
    geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
    geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
    theme_minimal(base_size = 14) + # clean minimal theme
    theme(
      panel.background = element_rect(fill = "#f7f7f7", color = NA), # light gray background
      panel.grid.major = element_line(color = "#dcdcdc"), # soft grid lines
...
graph_noOutliers <- function(x, y) {
  ...
  return(ggplot(z, aes(x = Growth, y = Gini_Coeff)) +
    geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
    geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
    theme_minimal(base_size = 14) + # clean minimal theme
    theme(
...
Bang_I_Ineq_and_Growth <- to_join(Bang_I_Growth_cleaned, Bang_I_Ineq_wider)
Bang_W_Ineq_and_Growth <- to_join(Bang_I_Growth_cleaned, Bang_W_Ineq_wider)
Ken_I_Ineq_and_Growth <- to_join(Ken_I_Growth_cleaned, Ken_I_Ineq_wider)
Ken_W_Ineq_and_Growth <- to_join(Ken_I_Growth_cleaned, Ken_W_Ineq_wider)
...
# Plotting the graphs (with outliers)
graph(Bang_I_Ineq_and_Growth, "Bangladesh Income Growth/Income Inequality")
graph(Bang_W_Ineq_and_Growth, "Bangladesh Income Growth/Wealth Inequality")
graph(Ken_I_Ineq_and_Growth, "Kenya Income Growth/Income Inequality")
graph(Ken_W_Ineq_and_Growth, "Kenya Income Growth/Wealth Inequality")

# Plotting the graphs (no outliers)
graph_noOutliers(Bang_I_Ineq_and_Growth, "Bangladesh Income Growth/Income Inequality (No Outliers)")
graph_noOutliers(Bang_W_Ineq_and_Growth, "Bangladesh Income Growth/Wealth Inequality (No outliers)")
graph_noOutliers(Ken_I_Ineq_and_Growth, "Kenya Income Growth/Income Inequality (No outliers)")
graph_noOutliers(Ken_W_Ineq_and_Growth, "Kenya Income Growth/Wealth Inequality (No outliers)")
```
...
```{r}
# Graphing Trade vs Gini for Bangladesh
ggplot(All_Data_Bang, aes(x = Bang_Trade, y = Gini_Coeff)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
...
# Graphing Trade vs Growth for Bangladesh
ggplot(All_Data_Bang, aes(x = Bang_Trade, y = Growth)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
...
# Graphing Trade vs Gini for Kenya
ggplot(All_Data_Ken, aes(x = Ken_Trade, y = Gini_Coeff)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
...
# Graphing Trade vs Growth for Kenya
ggplot(All_Data_Ken, aes(x = Ken_Trade, y = Growth)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
...
# Graphing Trade vs Gini for Bangladesh (no outliers)
ggplot(All_Data_Bang, aes(x = Bang_Trade_clean, y = Gini_Coeff_clean)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
  ...
  labs(
    title = "Bangladesh: Trade/Gini (no outliers)",
    x = "Current Account ($bn)",
    y = "Gini Coefficient"
  )

# Graphing Trade vs Growth for Bangladesh (no outliers)
ggplot(All_Data_Bang, aes(x = Bang_Trade_clean, y = Growth_clean)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
  ...
  labs(
    title = "Bangladesh: Trade/Growth (no outliers)",
    x = "Growth (%)",
    y = "Gini Coefficient"
  )

# Graphing Trade vs Gini for Kenya (no outliers)
ggplot(All_Data_Ken, aes(x = Ken_Trade_clean, y = Gini_Coeff_clean)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
  ...
  labs(
    title = "Kenya: Trade/Gini (no outliers)",
    x = "Current Account ($bn)",
    y = "Gini Coefficient"
  )

# Graphing Trade vs Growth for Kenya (no outliers)
ggplot(All_Data_Ken, aes(x = Ken_Trade_clean, y = Growth_clean)) +
  geom_point(color = "#1f77b4", size = 3, alpha = 0.7) + # points: muted blue
  geom_smooth(method = "lm", color = "#333333", size = 1.5) + # line: dark gray
  theme_minimal(base_size = 14) + # clean minimal theme
  theme(
  ...
  labs(
    title = "Kenya: Trade/Growth (no outliers)",
    x = "Growth (%)",
    y = "Gini Coefficient"
  )
```
...
All_Data_Bang_clean <- All_Data_Bang[, c("Bang_Trade_clean", "Gini_Coeff_clean")]
All_Data_Bang_clean <- na.omit(All_Data_Bang_clean)

All_Data_Ken_clean <- All_Data_Ken[, c("Ken_Trade_clean", "Gini_Coeff_clean")]
